...
public class GetActions extends AsyncTask<Void, Void, ArrayList<ArrayList<String>>> {

    public Context context;
    public Activity activity;
    public String id;
    public GetActions(Context ctx, Activity act, String userid){
        context = ctx;
        activity = act;
        id = userid;
    }
    ...
    protected ArrayList<ArrayList<String>> doInBackground(Void... voids) {
        ...
        try {
            String urlString = "http://45.134.255.158:80/actions?group_id="+id;
            Log.e("id: ",id);
            ...
            JSONArray triggers = answer.getJSONArray("actions");
            for (int i = 0;i<triggers.length();i++){
                ArrayList<String> tempok = new ArrayList<>();
                    JSONObject tempjson = new JSONObject(triggers.get(i).toString());

                   if (tempjson.get("type").toString().equals("0")){
                        tempok.add(tempjson.get("Удаление постов").toString());}
